def FKM_climatology(zm=None, z0=None, umean=None, ol=None, sigmav=None, ustar=None,
    ...
    if None in [zm, ol, sigmav, ustar] or (z0 is None and umean is None):
        raise_fkm_exception(1, verbosity)
    ...
    if any(len(lst) != ts_len for lst in [sigmav, wind_dir, ol]):
        ...
        raise_fkm_exception(11, verbosity)

    # Special treatment for zm, which is allowed to have length 1 for any
    # length >= 1 of all other parameters
    if all(val is None for val in zm): raise_fkm_exception(12, verbosity)
    if len(zm) == 1:
        raise_fkm_exception(17, verbosity)
    ...
    for ix, (ustar, sigmav, ol, wind_dir, zm, z0, umean) \
        ...
        valids[ix] = check_fkm_inputs(ustar, sigmav, ol, wind_dir, zm, z0, umean, rslayer, verbosity)
        #print ustar, sigmav, ol, wind_dir, zm, z0, umean, rslayer, verbosity
        # If inputs are not valid, skip current footprint
        if not valids[ix]:
            raise_fkm_exception(16, verbosity)
        else:
            ...
            #phi = numpy.zeros(shape=(x.shape[0],y.shape[1])) # set phi to zeros
    ...
    if n==0:
        ...
        print "number of valid footprints=",n
...
#===============================================================================
#===============================================================================
def check_fkm_inputs(ustar, sigmav, ol, wind_dir, zm, z0, umean, rslayer, verbosity):
    # Check passed values for physical plausibility and consistency
    if zm <= 0.:
        raise_fkm_exception(2, verbosity)
        return False
    if z0 <= 0.:
        raise_fkm_exception(3, verbosity)
        return False
    if float(zm)/ol < -3:
        raise_fkm_exception(7, verbosity)
        return False
    if float(zm)/ol > 3:
        raise_fkm_exception(7, verbosity)
        return False
    if sigmav <= 0:
        raise_fkm_exception(8, verbosity)
        return False
    if ustar <= 0.1:
        raise_fkm_exception(9, verbosity)
        return False
    if umean <= 0.0:
        raise_fkm_exception(21, verbosity)
        return False
    if wind_dir > 360:
        raise_fkm_exception(10, verbosity)
        return False
    if wind_dir < 0:
        raise_fkm_exception(10, verbosity)
        return False
...
def raise_fkm_exception(code, verbosity):
